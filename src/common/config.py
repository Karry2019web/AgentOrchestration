    ...
    def _load_env_overrides(self) -> None:
        ...
        for key, value in os.environ.items():
            if key.startswith(prefix):
                ...
                config_key = self._env_key_to_config_key(raw)
                self._set_nested(config_key, value)

    @staticmethod
    def _env_key_to_config_key(key: str) -> str:
        """Convert an AO_ env suffix to a dotted config key.

        Double underscore (__) represents a literal underscore in the key.
        Single underscore maps to a dot separator.

        Examples:
            API__URL  -> api_url  (literal underscore preserved)
            API_URL   -> api.url  (single underscore = dot separator)
        """
        return key.replace("__", "\x00").replace("_", ".").replace("\x00", "_")
