...
class ArtifactNotFoundError(AgentOrchestratorError):
    """Raised when an artifact cannot be read, including cross-region replication state."""
    def __init__(self, artifact_id: str, region: str, metadata_state: str):
        self.artifact_id = artifact_id
        self.region = region
        self.metadata_state = metadata_state
        super().__init__(f"Artifact {artifact_id} not found in region {region} (metadata state: {metadata_state})")
