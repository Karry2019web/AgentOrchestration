"""Orchestration Engine — Core execution and coordination logic with workflow deletion guard."""
...
class OrchestrationEngine:
    def __init__(self, max_workers: int = 10, agent_timeout: int = 300):
        ...
        # Wire scheduler into workflow manager for deletion notifications
        self.workflow_manager.register_scheduler(self.scheduler)
    ...
    def delete_workflow(self, workflow_id: str) -> bool:
        """Delete a workflow and block any new runs for it."""
    ...
    async def _execute_task(self, task: Dict[str, Any]) -> None:
        ...
        try:
            # Check if the task's workflow has been deleted before execution
            workflow_id = task.get("workflow_id")
            if workflow_id and self.scheduler.is_workflow_deleted(workflow_id):
                raise WorkflowDeletedError(
                    f"Task {task_id} belongs to deleted workflow {workflow_id}"
                )
        ...
        except WorkflowDeletedError:
            logger.warning(f"Skipping task {task_id} for deleted workflow {workflow_id}")
            self.scheduler.complete(task_id)
            for hook in self._hooks["on_error"]:
                await hook(task, WorkflowDeletedError(
                    f"Task {task_id} belongs to deleted workflow {workflow_id}"
                ))
