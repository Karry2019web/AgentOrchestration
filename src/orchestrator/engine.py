...
from src.orchestrator.dispatcher import EventDispatcher, DispatchRevision
...
class OrchestrationEngine:
    ...
    async def _execute_task(self, task: Dict[str, Any]) -> None:
        ...
        event = {
            "type": task.get("type", "task.submitted"),
            "revision": task.get("revision", {}),
        }
        if not self._event_dispatcher.dispatch(event):
            logger.warning(
                f"Task {task_id} rejected by event dispatcher "
                f"(quarantined={self._event_dispatcher.quarantine.count}, "
                f"rejected={self._event_dispatcher.rejected})"
            )
            return
