...
def cli():
    ...
    logs_parser.add_argument("--tail", "-t", type=int, default=50, help="Number of lines (max 10000)")
    ...
    if args.command == "init":
        ...
        if args.tail > MAX_LOG_TAIL:
            print(
                f"Error: --tail value {args.tail} exceeds maximum of {MAX_LOG_TAIL}",
                file=sys.stderr,
            )
            sys.exit(1)
