...
class TestTaskScheduler:
    ...
    def test_reject_missing_payload(self):
        with pytest.raises(MalformedPayloadError):
            self.scheduler.enqueue({"type": "test"})

    def test_reject_non_dict_task(self):
        with pytest.raises(MalformedPayloadError):
            self.scheduler.enqueue("legacy_string_record")

    def test_reject_non_dict_payload(self):
        with pytest.raises(MalformedPayloadError):
            self.scheduler.enqueue({"type": "test", "payload": "corrupted"})

    def test_skip_malformed_on_dequeue(self):
        self.scheduler.enqueue({"type": "good", "payload": {"data": 1}})
        import asyncio
        task = asyncio.run(self.scheduler.dequeue())
        assert task is not None
        assert task["type"] == "good"
    ...
    def test_idempotent_fail(self):
        ...
        task = asyncio.run(self.scheduler.dequeue())
        task_id = task["id"]
        assert self.scheduler.fail(task_id) is True
        assert self.scheduler.fail(task_id) is False

    def test_validate_payload_static(self):
        TaskScheduler.validate_payload({"payload": {}})
        TaskScheduler.validate_payload({"payload": []})
        with pytest.raises(MalformedPayloadError):
            TaskScheduler.validate_payload("string")
        with pytest.raises(MalformedPayloadError):
            TaskScheduler.validate_payload({"payload": "bad"})
        with pytest.raises(MalformedPayloadError):
            TaskScheduler.validate_payload({})
