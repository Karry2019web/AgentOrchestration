...
class TestConfig:
    ...
    def test_env_override_literal_underscore(self, tmp_path, monkeypatch):
        """Double underscore in env key maps to literal underscore."""
        config_file = tmp_path / "config.json"
        config_file.write_text('{"api_url": "https://default.example"}')
        monkeypatch.setenv("AO_API__URL", "https://env.example")
        config = Config(str(config_file))
        assert config.get("api_url") == "https://env.example"

    def test_env_override_single_underscore_is_dot(self, tmp_path, monkeypatch):
        """Single underscore maps to dot separator (backward compatible)."""
        config_file = tmp_path / "config.json"
        config_file.write_text('{"api": {"url": "https://default.example"}}')
        monkeypatch.setenv("AO_API_URL", "https://env.example")
        config = Config(str(config_file))
        assert config.get("api.url") == "https://env.example"

    def test_env_override_mixed_underscores(self, tmp_path, monkeypatch):
        """Mixed double/single underscores work correctly."""
        config_file = tmp_path / "config.json"
        config_file.write_text('{"endpoint_url": {"retries": 3}}')
        monkeypatch.setenv("AO_ENDPOINT__URL_RETRIES", "5")
        config = Config(str(config_file))
        assert config.get("endpoint_url.retries") == 5
        assert isinstance(config.get("endpoint_url.retries"), int)
